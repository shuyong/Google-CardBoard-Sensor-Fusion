...
class Matrix {
     ...
     */
    public: static void multiplyMM(float result_ref[], int resultOffset,
            float lhs_ref[], int lhsOffset, float rhs_ref[], int rhsOffset) {

        float* resultMat = &result_ref[resultOffset];
        const float* lhs = &lhs_ref[lhsOffset];
        const float* rhs = &rhs_ref[rhsOffset];
    ...
    private: static void multiplyMM(float* r, const float* lhs, const float* rhs) {
        for (int i=0 ; i<4 ; i++) {
    ...
    public: static void multiplyMV(float resultVec[],
        ...
        float* resultV = &resultVec[resultVecOffset];
        const float* lhs = &lhsMat[lhsMatOffset];
        const float* rhs = &rhsVec[rhsVecOffset];

        multiplyMV(resultV, lhs, rhs);
    }

    private: static void multiplyMV(float* r, const float* lhs, const float* rhs) {
        mx4transform(rhs[0], rhs[1], rhs[2], rhs[3], lhs, r);
    }

    private: static inline void mx4transform(float x, float y, float z, float w, const float* pM, float* pDest) {
    ...
    public: static void transposeM(float mTrans[], int mTransOffset, float m[],
        ...
        for (int i = 0; i < 4; i++) {
            ...
            mTrans[i + 4 * 0 + mTransOffset] = m[mBase + 0];
            mTrans[i + 4 * 1 + mTransOffset] = m[mBase + 1];
            mTrans[i + 4 * 2 + mTransOffset] = m[mBase + 2];
            mTrans[i + 4 * 3 + mTransOffset] = m[mBase + 3];
        }
    }
     ...
     */
    public: static bool invertM(float mInv[], int mInvOffset, float m[],
            int mOffset) {
    ...
    public: static void frustumM(float m[], int offset,
        ...
        if (near <= 0.0f) {
            fprintf(stderr, "near <= 0.0f");
	    return;
        }
        if (far <= 0.0f) {
            fprintf(stderr, "far <= 0.0f");
	    return;
        }
        const float r_width  = 1.0f / (right - left);
        const float r_height = 1.0f / (top - bottom);
