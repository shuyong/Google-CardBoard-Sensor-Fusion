...
#define HARDWARE_DLSYM(fptr, sym) do { if (_libhardware == NULL) { _init_lib_hardware(); }; if (*(fptr) == NULL) { *(fptr) = (void *) dlsym(_libhardware, sym); } } while (0) 

static void _init_lib_hardware()
{
#ifdef __LP64__
	_libhardware = (void *) dlopen("/system/lib64/libhardware.so", RTLD_LAZY);
#else
	_libhardware = (void *) dlopen("/system/lib/libhardware.so", RTLD_LAZY);
#endif
}
